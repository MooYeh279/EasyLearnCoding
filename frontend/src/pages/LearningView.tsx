...
import { Layout, Spin, Button, message, Modal } from 'antd';
...
export default function LearningView() {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 's') {
        ...
        // If a cell is being edited, blur the editor first to commit the
        // edit buffer into cellsRef synchronously before we save.
  ...
  // use refs in handlers to avoid stale closures over cells
  const handleCellChange = useCallback((cellId: string, updates: Partial<NotebookCell>) => {
    ...
    editingRef.current = false;  // commit is done, global Ctrl+S can run again
    doSave(updated);
  }, [doSave]);
  ...
  return (
    <Layout style={{ height: '100vh' }}>
                ...
                {(sec.lessons || []).map((les) => {
                  ...
                  return (
                    <div
                      ...
                      onMouseEnter={(e) => {
                        if (!isActive) (e.target as HTMLElement).style.background = '#f0f0f5';
                      }}
                      onMouseLeave={(e) => {
      ...
      {/* ================================================================
          CONTENT AREA
          ================================================================ */}
      <Content style={{ overflow: 'auto', background: C.pageBg }}>
        {lessonLoading ? (
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                ...
                <Button
                  ...
                  onClick={() => downloadMarkdown(currentLesson.title, cellsToMarkdown(cells))}
                  style={btnGhost}
                >
                  {t('learn.exportMd')}
                </Button>
            ...
            {cells.length === 0 ? (
              ...
              >
                <Button
                  ...
                  icon={<EditOutlined />}
                  onClick={() => addCell('markdown')}
                  style={{ borderRadius: C.radiusSm, color: C.textSecondary, borderColor: C.borderLight }}
                >
                  + Markdown
                </Button>
                <Button
